...
# This script to generate the relative abundance tables for different species of Sticta
...
physeq.sticta.core <- physeq.sticta.core.P %>%
  aggregate_taxa(level = "ASVID") %>% # Change the level to the desired taxonomic level (e.g. "Family")
  microbiome::transform(transform = "compositional") %>%
  psmelt() %>%
  filter(ASVID != "") %>%
  select(ASVID, Sample, Abundance) %>%
  distinct() %>%
  pivot_wider(names_from = Sample, values_from = Abundance, values_fn = list) %>%
  # Taking the mean of the list elements and ensuring values are numeric
  mutate(across(where(is.list), ~map_dbl(., ~if(is.list(.)) mean(unlist(.), na.rm = TRUE) else as.numeric(.)))) %>%
  replace(is.na(.), 0)
...
# Metadata
meta_st <- sample.data.frame(physeq.sticta)  # Ensure to use sample_data() to get metadata as a data frame
...
write.table(compiled_table, file = "Sticta/co-divergence/Reviewed/compiled_table_asv.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
